  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-6 shadow-sm relative">
      <div className="flex items-center gap-2 mb-2">
        ...
        <h3 className="text-lg font-semibold text-gray-900">{t("Summary Language")}</h3>
      </div>
      <p className="text-sm text-gray-600 mb-4">
        {t("Pin one language as the default for new meetings. Unpinned languages remain as quick-switch options in the summary generator. Auto uses the dominant transcript language.")}</p>
        ...
        {recents.map((code) => {
          ...
          return (
            ...
            >
              <button
                type="button"
                aria-label={t(isPinned ? 'Unpin {language} as default' : 'Pin {language} as default', {
                  language: labelForCode(code),
                })}
                aria-pressed={isPinned}
                title={t(isPinned ? 'Click to unset as default' : 'Click to set as default')}
      ...
      <p className="text-xs text-gray-400 mt-3">
        {pinned
          ? t('Default: {language} - click it again to unset. Max 5 quick-switch options.', {
              language: labelForCode(pinned),
            })
