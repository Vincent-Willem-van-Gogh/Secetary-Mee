...
export function ModelManager({
  ...
  useEffect(() => {
    ...
    const initializeModels = async () => {
      try {
        ...
        toast.error(t("Failed to load transcription models"), {
    ...
    const setupListeners = async () => {
      ...
      unlistenComplete = await listen<{ modelName: string }>(
        ...
        (event) => {
          ...
          toast.success(`${getModelIcon(model?.accuracy || 'Good')} ${t('{model} ready!', { model: displayName })}`, {
            description: t("Model downloaded and ready to use"),
            duration: 4000
          });
      ...
      unlistenError = await listen<{ modelName: string; error: string }>(
        ...
        (event) => {
          ...
          toast.error(t('Failed to download {model}', { model: displayName }), {
            description: error,
            duration: 6000,
            action: {
              label: t('Retry'),
  ...
  const cancelDownload = async (modelName: string) => {
    ...
    try {
      ...
      toast.info(t('{model} download cancelled', { model: displayName }), {
        duration: 3000
      });
    } catch (err) {
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      ...
      {selectedModel && (
        ...
        >
          {t("Using")}{getDisplayName(selectedModel)} {t("for transcription")}</motion.div>
      )}
    </div>
  );
}
        ...
        <div className="flex items-start justify-between mb-2">
          <div className="flex-1">
            ...
            <div className="flex items-center space-x-4 text-sm text-gray-600 ml-9 mt-1.5">
              ...
              <span className="flex items-center space-x-1">
                <span>🎯</span>
                <span>{model.accuracy} {t("accuracy")}</span>
              </span>
              <span className="flex items-center space-x-1">
                <span>⚡</span>
                <span>{model.speed} {t("processing")}</span>
              </span>
            </div>
          </div>
