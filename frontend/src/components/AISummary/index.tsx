...
export const AISummary = ({ summary, status, error, onSummaryChange, onRegenerateSummary, meeting }: Props) => {
  ...
  const convertToMarkdown = () => {
    ...
    markdown += `## Date: ${meeting?.created_at ? new Date(meeting.created_at).toLocaleDateString(locale) : new Date().toLocaleDateString(locale)}\n\n`;
      ...
      {contextMenu.visible && selectedBlocks.length > 0 && (
        ...
        >
          ...
          >
            ...
            <span>{t("Copy")}{selectedBlocks.length > 1 ? `${selectedBlocks.length} blocks` : 'block'}</span>
          </button>
          <button
          ...
          >
            <span>🗑️</span>
            <span>{t("Delete")}{selectedBlocks.length > 1 ? `${selectedBlocks.length} blocks` : 'block'}</span>
          </button>
        </div>
      )}
