...
if not exist "whisper.cpp" (
    echo Vendored source directory 'whisper.cpp' not found
    goto :eof
)
...
if not exist "examples\server" (
    echo Server directory not found in vendored whisper.cpp source
    cd ..
    goto :eof
)
...
if not exist "examples\server\server.cpp" (
    echo Server source files not found in vendored whisper.cpp source
    cd ..
    goto :eof
)
